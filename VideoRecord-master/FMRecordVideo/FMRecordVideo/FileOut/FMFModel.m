...
//    [[UIApplication sharedApplication] addObserver:self forKeyPath:@"statusBarOrientation" options:NSKeyValueObservingOptionNew context:NULL];
}
...
//    [self updateCaptureVideoOrientation];
    ...
    // 3.2 必须在3.3之后加入，否则captureConnection值为nil
    // 3.2设置输出对象的一些属性
...
//    NSLog(@"befor deviceOrientation => %@,  videoOrientation => %@, previewlayerOrientation => %@",@(deviceOrientation),@([self.FileOutput connectionWithMediaType:AVMediaTypeVideo].videoOrientation),@([self.previewlayer connection].videoOrientation));
//    if ([[self.previewlayer connection] isVideoOrientationSupported]) {
...
//    }
//
//     NSLog(@"after deviceOrientation => %@,  videoOrientation => %@, previewlayerOrientation => %@",@(deviceOrientation),@([self.FileOutput connectionWithMediaType:AVMediaTypeVideo].videoOrientation),@([self.previewlayer connection].videoOrientation));
//}
